      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-slate-950 rounded-[28px] shadow-2xl flex flex-col max-h-[90vh] animate-slide-up border border-white/5 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
             <div className="space-y-0.5">
                <span className={inputLabelClass}><ShieldCheck size={8} /> Protocol Type</span>
             ...
             <div className="space-y-0.5">
                <span className={inputLabelClass}><CreditCard size={8} /> Classification</span>
                <div className="relative">
                  <select 
                    value={category} 
          ...
          <div className="space-y-0.5">
            <span className={inputLabelClass}><Tag size={8}/> Registry Label</span>
            <input 
              type="text" 
              value={name} 
        ...
        <div className="p-4 border-t dark:border-slate-800/60 bg-white dark:bg-slate-950 shrink-0">
          <div className="flex gap-2">
             {isEditing && onDelete && (
               <button onClick={() => { triggerHaptic(); if(window.confirm('Decommission this account?')) onDelete(initialData!.id); }} className="p-3 bg-rose-50 dark:bg-rose-950/20 text-rose-500 rounded-xl active:scale-90 transition-all">
                  <Trash2 size={18} />
               </button>
             )}
