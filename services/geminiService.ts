...
export async function auditTransaction(expense: Expense, currency: string) {
  const prompt = `
    ...
    Identify the correct main Category (Needs/Wants/Savings/Uncategorized) and a specific Sub-Category.
    Return JSON: {
      isCorrect: boolean,
    ...
    const response = await withRetry<GenerateContentResponse>(() => ai.models.generateContent({
      ...
      config: { 
        ...
        responseSchema: {
          ...
          required: ["isCorrect", "suggestedCategory", "suggestedSubCategory", "insight", "isAnomaly"]
        }
      }
    }));
    return JSON.parse(response.text || '{}');
